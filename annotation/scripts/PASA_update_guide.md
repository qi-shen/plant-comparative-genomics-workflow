...
1. **基因组序列** (FASTA格式)
   - 文件: `annotation/T01/BH_genome.masked.fa`

2. **转录组GTF文件**
   - 文件: `annotation/T01/transcriptome/BH_merged.gtf`

3. **现有注释** (可选)
   - 文件: `annotation/T01/structure/BH_final.gff3`
...
# 基因组文件
GENOME_FILE=${PROJECT_ROOT}/annotation/T01/BH_genome.masked.fa

# 转录组GTF文件
TRANSCRIPT_GTF=${PROJECT_ROOT}/annotation/T01/transcriptome/BH_merged.gff3
...
```bash
# 使用gffread转换
gffread -E annotation/T01/transcriptome/BH_merged.gtf \
        -o annotation/T01/transcriptome/BH_merged.gff3
```
...
PASA.pl \
    -c pasa_align.config \
    -C -R \
    -g annotation/T01/BH_genome.masked.fa \
    -t annotation/T01/transcriptome/BH_merged.gff3 \
    --ALIGNERS gmap,blat \
    --CPU 32
```
...
PASA.pl \
    ...
    -g annotation/T01/BH_genome.masked.fa \
    -t annotation/T01/transcriptome/BH_merged.gff3 \
    -u annotation/T01/structure/BH_final.gff3 \
    --ALIGNERS gmap,blat \
    --CPU 32
```
...
cp pasa_assemblies.gff3 annotation/T01/structure/BH_pasa_updated.gff3
```
...
```bash
# 比较现有注释和转录组
gffcompare -r annotation/T01/structure/BH_final.gff3 \
           -o comparison \
           annotation/T01/transcriptome/BH_merged.gtf
...
stringtie --merge \
          ...
          -G annotation/T01/structure/BH_final.gff3 \
          gtf_list.txt
```
...
taco_run -p 32 \
         -o taco_output \
         annotation/T01/structure/BH_final.gtf \
         annotation/T01/transcriptome/BH_merged.gtf
```
...
```bash
# 提取更新后的蛋白质序列
gffread -y annotation/T01/structure/BH_pasa_updated.gff3 \
        -g annotation/T01/BH_genome.masked.fa \
        -x annotation/T01/structure/BH_pasa_updated.cds.fa

# 翻译为蛋白质
transeq annotation/T01/structure/BH_pasa_updated.cds.fa \
         annotation/T01/structure/BH_pasa_updated.pep.fa
