...
BASE = Path("/path/to/project_root/comparative_genomics")
...
RULES: Dict[str, Callable[[str], str]] = {
    ...
    "ATH": root_by_regex(r"^(ATH_transcript:[^.]+)\.\d+$"),
    "CQU": root_by_regex(r"^(CQU_.+)\.\d+$"),
    "SMO": root_by_regex(r"^(SMO_[^.]+)\.\d+$"),
    "VVI": root_by_regex(r"^(VVI_.+)\.t\d+$"),
    "TCH": root_by_regex(r"^(TCH_TC\d+G\d+)T\d+$"),
    "HAM": root_by_regex(r"^(HAM_.+)\.t\d+$"),
    "GPA": root_by_regex(r"^(GPA_.+)\.m\d+$"),
    "FMU": root_identity,  # 新版HR基因组无转录本后缀，35926基因=35926蛋白
    "APA": root_by_regex(r"^(APA_.+)-mRNA-\d+$"),
    "RSO": root_by_regex(r"^(RSO_.+)\.p\d+$"),
