...
species_cn <- c(
  T01="T01BH", T02="T02CK", C02="TAUC02", C03="TCHC03", C01="RSOC01",
  C04="APAC04", O01="ATHO01", C05="CQUC05", C06="DCAC06", C08="FMUC08",
  C07="GPAC07", C09="HAMC09", C10="POLC10", C11="SMOC11", O02="VVIO02")
species_colors <- setNames(rep(palette14, length.out=15), species_order)

D <- "${PROJECT_ROOT}/comparative_genomics/Comparative_Genomics_Results"
...
tip_cols["T01"] <- palette14[5]; tip_cols["T02"] <- palette14[2]
...
if(length(sp5_cols) > 0 && "C03" %in% names(sp5_cols)) {
  tch_col <- sp5_cols[["C03"]]
