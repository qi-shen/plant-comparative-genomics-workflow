...
ks_colors <- c(
  BH = "#00BFAE", CK = "#1F77B4", TAU = "#9467BD",
...
# ============================================================
# Panel B: 共线性点图 (4个: BH vs CK/TAU/TCH/RSO, 2x2布局)
# ============================================================
...
# 4个配对
pairs <- list(c("BH", "CK"), c("BH", "TAU"), c("BH", "TCH"), c("BH", "RSO"))
dot_plots <- list()

bed_bh_data <- read_bed("BH")
...
for (pr in pairs) {
  ...
  p <- ggplot(merged, aes(x = x / 1e6, y = y / 1e6, color = chr1)) +
    ...
    scale_y_continuous(breaks = chr_breaks_y$pos / 1e6,
                       ...
                       expand = c(0.02, 0)) +
    labs(x = paste0("BH (", "T. sp. BH", ") chromosome"),
         y = paste0(ks_labels[sp2], " chromosome"),
         title = paste0("BH vs ", ks_labels[sp2])) +
    theme_pub(9) +
    theme(
