...
# 从GFF文件提取染色体信息
gff_files = {
    "BH": "/path/to/project_root/new_anno/BH.final.gff3",
    "CK": "/path/to/project_root/new_anno/CK.final.gff3"
}
...
anchors_file = "../05_synteny/jcvi_plots/BH.CK.anchors"
if os.path.exists(anchors_file):
    ...
    with open(anchors_file) as f:
        for line in f:
            ...
            if len(parts) >= 3:
                gene1 = parts[0]  # BH gene
                gene2 = parts[1]  # CK gene
...
for sp in ["BH", "CK"]:
    gff = f"/path/to/project_root/new_anno/{sp}.final.gff3"
...
if (file.exists("data/BH_gene_density.txt")) {
    ...
    # 合并数据
    bh_density$species <- "BH"
    ck_density$species <- "CK"
    ...
    p <- ggplot(plot_data, aes(x = start/1e6, y = count, fill = species)) +
        ...
        scale_fill_manual(values = c("BH" = "#D62728", "CK" = "#1F77B4")) +
        labs(x = "Position (Mb)", y = "Gene count per 500kb",
             title = "Gene Density Distribution: BH vs CK") +
        theme_minimal() +
        theme(
