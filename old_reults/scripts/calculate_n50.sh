...
main() {
    ...
    # BH和CK
    if [ -f "${RESULTS_DIR}/targets 测序组装物种 T01_T02/基因组/BH.Chr.final.fa.gz" ]; then
        # 临时解压用于计算
        log_info "临时解压BH基因组..."
        gunzip -c "${RESULTS_DIR}/targets 测序组装物种 T01_T02/基因组/BH.Chr.final.fa.gz" > /tmp/BH_temp.fa
        species_genomes["BH"]="/tmp/BH_temp.fa"
    elif [ -f "${RESULTS_DIR}/targets 测序组装物种 T01_T02/基因组/BH.Chr.final.fa" ]; then
        species_genomes["BH"]="${RESULTS_DIR}/targets 测序组装物种 T01_T02/基因组/BH.Chr.final.fa"
    fi
    
    if [ -f "${RESULTS_DIR}/targets 测序组装物种 T01_T02/基因组/CK.Chr.final.fa.gz" ]; then
        log_info "临时解压CK基因组..."
        gunzip -c "${RESULTS_DIR}/targets 测序组装物种 T01_T02/基因组/CK.Chr.final.fa.gz" > /tmp/CK_temp.fa
        species_genomes["CK"]="/tmp/CK_temp.fa"
    elif [ -f "${RESULTS_DIR}/targets 测序组装物种 T01_T02/基因组/CK.Chr.final.fa" ]; then
