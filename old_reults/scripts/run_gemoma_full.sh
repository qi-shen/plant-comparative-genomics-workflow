...
process_species() {
    ...
    ref_peps["ganmeng"]="${RESULTS_DIR}/C02/tau.longest_pep.fasta"
    
    ref_gffs["chinensis"]="${RESULTS_DIR}/C03/Tchinensis.gff3"
    ref_peps["chinensis"]="${RESULTS_DIR}/C03/Tchinensis_pep.fa"
    
    local gemoma_predictions=()
    
    for ref_id in hongsha ganmeng chinensis; do
