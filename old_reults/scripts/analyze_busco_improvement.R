...
# 分析两个样本
bh_result <- analyze_busco_status("BH")
ck_result <- analyze_busco_status("CK")
...
if (!is.null(bh_result)) {
  cat(sprintf("BH: Complete %.2f%%, Fragmented %.2f%%, Missing %.2f%%\n",
...
if (!is.null(ck_result)) {
  cat(sprintf("CK: Complete %.2f%%, Fragmented %.2f%%, Missing %.2f%%\n",
...
if (!is.null(bh_result) && !is.null(ck_result)) {
  ...
  cat(sprintf("\n如果修复所有片段化基因:\n"))
  cat(sprintf("  BH: 可从 %.2f%% 提升到 %.2f%%\n",
              bh_result$stats$complete/bh_result$stats$total*100,
              bh_potential))
  cat(sprintf("  CK: 可从 %.2f%% 提升到 %.2f%%\n",
              ck_result$stats$complete/ck_result$stats$total*100,
              ck_potential))
  
  cat(sprintf("\n目标: >90%% Complete\n"))
  cat(sprintf("  BH: 需要修复 %.0f 个片段化基因 (%.1f%%)\n",
              bh_result$stats$fragmented * 0.2, 20))
  cat(sprintf("  CK: 需要修复 %.0f 个片段化基因 (%.1f%%)\n",
