...
```bash
# 提取片段化BUSCO ID
grep "Fragmented" annotation/evaluation/busco/BH/BH/run_embryophyta_odb10/full_table.tsv | cut -f1 > fragmented_busco_ids.txt
```
...
GENOME="annotation/BH/BH_genome.masked.fa"
BAM="annotation/BH/transcriptome/BH_merged.bam"
PROTEINS="databases/uniprot_sprot.fasta"
OUTPUT="annotation/BH/braker2"
...
如果成功修复所有片段化基因：
- **BH**: 可从 88.3% 提升到 **96.1%** (88.3% + 7.8%)
- **CK**: 可从 87.6% 提升到 **96.1%** (87.6% + 8.5%)

如果修复50%的片段化基因：
- **BH**: 可提升到 **92.2%** (88.3% + 3.9%)
- **CK**: 可提升到 **91.9%** (87.6% + 4.25%)
