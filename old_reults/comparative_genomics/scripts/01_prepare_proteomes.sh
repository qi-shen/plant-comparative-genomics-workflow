...
# 创建输出目录
OUTDIR="old_reults/comparative_genomics/01_proteomes"
mkdir -p $OUTDIR
...
for sp in "${!SPECIES[@]}"; do
    ...
    if [ -f "$src" ]; then
        # 复制并重命名序列ID (添加物种前缀)
        # 使用seqkit处理FASTA文件
        conda run -n comparative seqkit replace -p '^' -r "${sp}_" "$src" > "$dst" 2>/dev/null || \
        # 如果seqkit失败，使用awk处理
        awk -v sp="$sp" '/^>/{gsub(/^>/, ">" sp "_"); print; next} {print}' "$src" > "$dst"
